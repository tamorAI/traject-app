...
export type T_API_ENV = Static<typeof apiEnvSchema>;

export function validateEnv(): T_API_ENV {
  const rawCorsOrigin = process.env["CORS_ORIGIN"];
  const corsOriginArray = rawCorsOrigin
    ? rawCorsOrigin.split(",").map((origin: string) => origin.trim())
  ...
  if (!Value.Check(apiEnvSchema, rawEnv)) {
    const errors = [...Value.Errors(apiEnvSchema, rawEnv)];
  ...
  return Value.Decode(apiEnvSchema, rawEnv);
}

export const API_ENV = validateEnv();
